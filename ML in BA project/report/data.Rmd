...
```{r}
autoplot <- df %>% summarise(date = gsub( ".*(\\d{4}/\\d{1,2}/\\d{1,2}).*", "\\1", df$url)) 

library(fpp3)

tsibble <- cbind(autoplot, y = df$shares)
articlebyday <- tsibble %>% group_by(date) %>% summarise(n()) #number of articles published by day

articlebyday$date <-  as.Date(articlebyday$date)

articlebyday %>% as_tsibble() %>% autoplot
```
